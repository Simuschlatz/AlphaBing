...
class Legal_move_generator:
        ...
        for current_square in cls.board.piece_lists[cls.friendly][Piece.rook]:
            ...
            for dir_idx, squares_in_dir in rook_move_map.items():
                ...
                for target_square in squares_in_dir:
                    ...
                    captures_checking_cannon = cls.cause_cannon_defect.get(current_square, None) == target_square
    ...
    @classmethod
    def blocks_all_checks_orth(cls, current_square, target_square):
        """
        Checking if an orthogonally moving piece is blocking all existing checks in current state of game
        """
        num_checks_blocked = cls.block_check_hash.get(target_square, 0) 
        # - target_square in cls.checking_cannons
    ...
    @classmethod
    def dir_idx_same_file(cls, rank_1, rank_2):
        d_rank = rank_2 - rank_1
        return cls.dir_offsets.index(9 * d_rank / abs(d_rank))

    @classmethod
    def dir_idx_same_rank(cls, file_1, file_2):
        d_file = file_2 - file_1
        return cls.dir_offsets.index(d_file / abs(d_file))
            ...
            for attacking_square in targets_in_dir:
                ...
                if Piece.is_color(piece, cls.opponent) and Piece.is_type(piece, Piece.cannon):
                    ...
                    if screens:
                        ...
                        cls.cause_cannon_defect[next(iter(friendly_screens))] = attacking_square
