...
def load_moves() -> list:
    ...
    generate_quiets = generate_quiets
    moves = []
    init()
...
def init(piece_l, sq, colors, ):
    """
    Initializes data used for move generation
    """
    global moving_king, opponent_king, piece_lists, moves, attack_map, illegal_squares, pinned_squares, checks, \
        block_check_hash, checking_cannon_square, double_screens, cause_cannon_defect
    piece_lists = piece_l
...
def bitvector_legal_moves(legal_moves=None, flip_moves=False, action_space_vector=PrecomputingMoves.action_space_vector):
    """
    ...
    """
    legal_moves = legal_moves or load_moves()
    # if flip_moves: legal_moves = flip_moves(legal_moves)
